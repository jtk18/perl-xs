...
unsafe fn rethrow_panic(perl: Interpreter, e: Box<dyn any::Any>) -> ! {
    ...
    if errsv.is_null() {
        errsv = unsafe { make_error_sv(perl, "unknown typed panic inside Rust code") };
    }

    unsafe { perl.croak_sv(errsv) };
    unreachable!();
}
